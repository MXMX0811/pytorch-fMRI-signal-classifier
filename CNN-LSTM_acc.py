...
class CnnLSTM(nn.Module):
    def __init__(self, in_dim, hidden_dim, bidirectional, n_layer, n_class):
        ...
        self.lstm = nn.LSTM(in_dim, hidden_dim, n_layer, bidirectional=bidirectional,
                            batch_first=True)
        if bidirectional:
            self.fc1 = nn.Sequential(nn.Linear(hidden_dim*2, 64))
            self.fc2 = nn.Sequential(nn.Linear(64, n_class))
        else:
...
train_x, train_y = dataReader([1,3,5,6,7,8,9,10,13,14,15,18,21,22,23],['mot_1'])
test_x, test_y = dataReader([6],['mot_1'])
test_x = torch.from_numpy(test_x)
test_x = torch.tensor(test_x, dtype=torch.float32)
test_y = torch.from_numpy(test_y)

smo = RandomOverSampler(random_state=42)    # SMOTE处理样本数量不对称
...
for epoch in range(epochs):
    ...
    for b_x, b_y in zip(data_x,data_y):
        ...
        # if int(torch.max(output, 1)[1]) - int(b_y):   # 因样本不平衡加上权重
        #     loss = loss*10
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
