...
for epoch in range(1,epochs+1):
    ...
    for i in range(0, len(train_x), batch_size):
        ...
        for b_x, b_y in zip(train_x[i:i+batch_size], train_y[i:i+batch_size]):
            b_x = b_x.reshape(-1, 12, NUM_IC)
            b_x = b_x.reshape(-1, 1, 12, NUM_IC)
            b_y = b_y.reshape(-1)
    ...
    with torch.no_grad():
        ...
        for t_x, t_y in zip(test_x, test_y):
            t_x = t_x.reshape(-1, 12, NUM_IC)
            t_x = t_x.reshape(-1, 1, 12, NUM_IC)
            t_y = t_y.reshape(-1)
            ...
            if predicted == 1 and t_y[0] == 1:
                test_TP += 1
            elif predicted == 1 and t_y[0] == 0:
                test_FP += 1
            elif predicted == 0 and t_y[0] == 1:
                test_FN += 1
