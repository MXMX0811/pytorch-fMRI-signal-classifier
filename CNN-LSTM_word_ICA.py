    ...
    def __init__(self):
        ...
        self.conv = torch.nn.Sequential(
            torch.nn.Conv1d(
                ...
                padding=0),
            torch.nn.ReLU(),
            torch.nn.Conv1d(32, 64, kernel_size=5, padding=0),
            torch.nn.ReLU(),
            torch.nn.Conv1d(64, 128, kernel_size=3, padding=0),
            torch.nn.ReLU(),
            torch.nn.Conv1d(128, 256, kernel_size=3, padding=0),
            torch.nn.ReLU(),
            torch.nn.Conv1d(256, 512, kernel_size=3, padding=0),
            torch.nn.ReLU(),
        )
...
def dataReader(sub, task):
    ...
    path_input = '/home/zmx/ds002311/ICAtest/' + task + '/'
    if sub < 10:
        num = 'sub-0' + str(sub)
    else:
        num = 'sub-' + str(sub)
    ...
    img = nib.load('/home/zmx/ds002311/ICAtest/ICATest_sub01_timecourses_ica_s1_.nii')
    img = np.array(img.get_fdata())

    with open(path_label + label_name + '.tsv','rt') as csvfile:
        reader = csv.DictReader(csvfile, delimiter='\t')
        cond = [row['acc'] for row in reader]
    ...
    for task in ['mot_1']:
        for sub in [6]:
            ...
            for b_x, b_y in zip(train_x,train_y):
                ...
                if int(b_y) == 0 and int(torch.max(output, 1)[1]) != 0:
                    loss = loss*3
