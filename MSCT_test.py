...
for epoch in range(epochs):
    ...
    for i in range(0, len(train_x), batch_size):
        ...
        #     plt.xlabel(u"Number of train batches")
        #     plt.ylabel(u"Cumulative accuracy")
        #     t_now += 5        
...
X_train_tsne = X_train_tsne.reshape(X_train_tsne.shape[0], 32)
X_train_tsne = tsne.fit_transform(X_train_tsne)
x_min, x_max = X_train_tsne.min(0), X_train_tsne.max(0)
X_norm = (X_train_tsne - x_min) / (x_max - x_min)  # 归一化
plt.subplot(1,2,1)
...
X_test_tsne = X_test_tsne.reshape(X_test_tsne.shape[0], 32)
X_test_tsne=tsne.fit_transform(X_test_tsne)
x_min, x_max = X_test_tsne.min(0), X_test_tsne.max(0)
X_norm = (X_test_tsne - x_min) / (x_max - x_min)  # 归一化
plt.subplot(1,2,2)
plt.cla()
