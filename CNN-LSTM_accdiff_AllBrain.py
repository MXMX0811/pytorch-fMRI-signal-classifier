...
class CnnLSTM(nn.Module):
    def __init__(self):
        ...
        self.fc4 = nn.Sequential(nn.Linear(64, 32))
        self.fc5 = nn.Sequential(nn.Linear(32, 6))
...
def dataReader(sub, task):
    ...
    with open(path_label + label_name + '.tsv','rt') as csvfile:
        reader = csv.DictReader(csvfile, delimiter='\t')
        acc = [row['acc'] for row in reader]
        
    for i in range(len(cond)):
        if cond[i] == 'targ_easy' and acc[i] == '1':
            cond[i] = 0
        elif cond[i] == 'targ_easy' and acc[i] == '0':
            cond[i] = 1
        elif cond[i] == 'targ_hard' and acc[i] == '1':
            cond[i] = 2
        elif cond[i] == 'targ_hard' and acc[i] == '0':
            cond[i] = 3
        elif cond[i] == 'lure_hard' and acc[i] == '1':
            cond[i] = 4
        elif cond[i] == 'lure_hard' and acc[i] == '0':
            cond[i] = 5
    label.extend(cond)
